	...
	public function input($fieldName, $options = array()) {
		...
		if (!isset($options['before'])) {
			...
			if ($state) {
				...
				if (isset($this->validationErrors[$this->_modelForm][$fieldName])) {
					$options['before'] = '<div class="form-group has-error">';
				} else {
		...
		if ($this->_getFormType() == 'horizontal' || ($date && $formType == 'horizontal')) {
			...
			if ($options['after'] == '</div>') {
				if ($help && !empty($help)) {
					...
					$errorList = '';
					if (isset($this->validationErrors[$this->_modelForm][$fieldName])) {
						foreach ($this->validationErrors[$this->_modelForm][$fieldName] as $errorMessage) {
							$errorList = $errorList . '<small class ="help-block">' . $errorMessage . '</small>';
						}
					}
					$options['after'] = $errorList . '</div></div>';
				}
			}
		}
